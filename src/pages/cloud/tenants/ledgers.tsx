...
export function CloudTenantLedgers() {
  ...
  const { val: listLedgers } = cloud.useListLedgersByTenant(tenantId!);
    ...
    <div className="flex h-full">
      ...
      <div className="w-64 border-r border-[--border] bg-[--muted] p-4">
        ...
        <div className="space-y-1">
          {listLedgers.ledgers.map((ledger) => (
            <Link
              key={ledger.ledgerId}
              to={ledger.ledgerId}
...
export function CloudTenantLedgersIndex() {
  ...
  return (
    <div className="p-6">
      ...
      {listLedgers.ledgers.length === 0 ? (
        <p className="text-[--muted-foreground]">No ledgers yet. Create your first one!</p>
      ) : (
